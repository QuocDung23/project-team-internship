...
def publish_safety_event(
    ...
    details = {
        ...
        "consecutive_frame_count": int(consecutive_frame_count),
        "ear_threshold": float(EAR_PERSONAL_THRESHOLD if (not USE_CNN and calibrated) else EAR_THRESHOLD),
        "mar_threshold": float(MAR_THRESHOLD),
        "pitch_delta_threshold": float(_pose_delta_threshold),
        "cnn_closed_probability": float(_ema_cnn_conf) if USE_CNN else None,
        "cnn_enabled": bool(USE_CNN),
        "alarm_triggered": bool(alarm_triggered),
        "camera_id": f"camera-{ARGS.camera}",
    }
    ...
    while True:
        ...
        if mar_alert and not prev_mar_alert:
            publish_safety_event(
                ...
                severity="medium",
                confidence=min(1.0, MAR_COUNTER / max(1, MAR_CONSEC_FRAMES)),
                duration_ms=int((MAR_COUNTER / max(1.0, fps)) * 1000),
        ...
        if pose_alert and not prev_pose_alert:
            publish_safety_event(
                ...
                severity="medium",
                confidence=min(1.0, POSE_COUNTER / max(1, POSE_CONSEC_FRAMES)),
                duration_ms=int((POSE_COUNTER / max(1.0, fps)) * 1000),
                detection_method="head_pose_dlib",
                ear_value=float(ear),
                mar_value=float(mar),
                pitch_value=float(pitch),
